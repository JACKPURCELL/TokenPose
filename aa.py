...
class MaskGate(nn.Module):
    ...
    def __init__(self, patch_size, patch_num, input_channel, output_channel, running_mode=RunningMode.FineTuning,
                 dim=256, depth=4, kernel_size=9, fc_dim=4096, fc_depth=0, expand_dim=192):
        """
    ...
    def forward(self, feature, x):
        ...
        # current_input = torch.cat((feature, previous_input), 1)
        mask = self.convmixer(feature)
        ...
        if mask_mode == MaskMode.Positive:
            ...
            m_cfg.skipped_mac += current_mac * current_skip_rate.item()
            # mac_gates = n * h_out * w_out * c_in * 1 * k_h * k_w (n = batch_size, h_out = (h + 2*p - k)/s + 1 = h + 0 - 1 / 1
            m_cfg.skipped_patch += current_skip.item()
            m_cfg.total_patch += current_total_patch

        if self.vis:
            m_cfg.mask = mask.clone().detach()
        # mask = mask.expand(feature.shape[0], feature.shape[1], mask_h, mask_w)
        ...
        mask = mask.repeat(1, 1, self.expand_dim)
